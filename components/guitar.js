...
export default function Guitar({ guitar }) {
    ...
    return (
        <div className='grid lg:grid-cols-2 gap-4 place-items-center items-center max-w-[70rem] m-auto'>
            <Image src={image.data.attributes.formats.medium.url} width={600} height={400} alt={`Imagen guitarra ${name}`} className='w-72 '/>
            <div className='p-4 lg:w-64 xl:w-full overflow-hidden'>
                ...
                <Link href={`/guitars/${url}`} className='mt-8 block bg-black text-white p-4 text-center uppercase text-[2rem] font-bold hover:bg-primary transition duration-300'>Ver producto</Link>
            </div>
        </div>
    )
}
